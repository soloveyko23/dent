    ...
    (() => {
        ...
        function initSliders() {
            if (document.querySelector(".swiper")) {
                ...
                new swiper_core_Swiper(".reviews-specialist__slider", {
                    modules: [ Navigation, Pagination ],
                    observer: true,
                    observeParents: true,
                    speed: 800,
                    ...
                    lazy: true,
                    pagination: {
                        el: ".reviews-specialist .swiper-pagination",
                        clickable: true
                    },
                    navigation: {
                        prevEl: ".reviews-specialist .swiper-button-prev",
                        nextEl: ".reviews-specialist .swiper-button-next"
                    },
                    breakpoints: {
                        320: {
                            slidesPerView: 1.1,
                ...
                new swiper_core_Swiper(".certificates__slider", {
                    modules: [ Navigation, Pagination ],
                    observer: true,
                    observeParents: true,
                    speed: 800,
...
***************************************************************************** */
        ...
        da.init();
        const togglePhoneBlock = () => {
            const phoneBlock = document.querySelector(".phone-block");
            if (!phoneBlock) {
                console.error('Не найден блок "phone-block"');
                return;
            }
            const toggleButton = document.querySelector(".phone-block__button");
            toggleButton.addEventListener("click", (() => {
                phoneBlock.classList.toggle("open");
            }));
            document.addEventListener("click", (event => {
                const target = event.target;
                if (!phoneBlock.contains(target) && !target.classList.contains("phone-block__button")) phoneBlock.classList.remove("open");
            }));
        };
        const toggleHeaderServices = () => {
            const headerServices = document.querySelector(".header__services");
            if (!headerServices) {
                console.error('Не найден блок "header__services"');
                return;
            }
            const toggleButton = document.querySelector(".button-services");
            toggleButton.addEventListener("click", (() => {
                headerServices.classList.toggle("open");
            }));
            document.addEventListener("click", (event => {
                const target = event.target;
                if (!headerServices.contains(target) && !target.classList.contains("button-services")) headerServices.classList.remove("open");
            }));
        };
        document.addEventListener("DOMContentLoaded", toggleHeaderServices);
        document.addEventListener("DOMContentLoaded", togglePhoneBlock);
        function init() {
            if (window.innerWidth >= 1024 && !("ontouchstart" in window)) new script_SmoothScroll(document, 100, 20);
        }
